<template>
    <div class="mb-6">
        ...
        <div v-if="showCommandModal" class="fixed inset-0 z-50 flex items-center justify-center p-4">
            <div class="absolute inset-0 bg-black/50" @click="closeModal"></div>
            <div class="relative bg-white rounded-xl shadow-xl w-full max-w-md p-6">
                <h2 class="text-lg font-semibold text-gray-800 mb-4">Отправить команду</h2>
                ...
                <div class="space-y-4">
                    ...
                    <div>
                        <div class="flex items-center justify-between mb-1">
                            <label class="block text-sm font-medium text-gray-700">Аргументы</label>
                        ...
                        <p v-if="args.length === 0" class="text-xs text-gray-400 italic">Нет аргументов</p>
                    </div>
                ...
                <div class="flex justify-end gap-2 mt-6">
                    ...
                    <button @click="submitCommand" :disabled="sending || !command.trim()"
    ...
    try {
        ...
        for (const arg of args.value) {
            if (arg.key.trim()) {
                ...
                argsObj[arg.key.trim()] = !isNaN(numVal) && arg.value !== '' ? numVal : arg.value
            }
        }
        await sendDeviceCommand(props.deviceId, command.value.trim(), argsObj)
        closeModal()
    } catch (e: any) {
        error.value = e?.message || 'Ошибка отправки команды'
    } finally {
        sending.value = false
    }
}
