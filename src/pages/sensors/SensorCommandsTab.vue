    ...
    <div class="bg-white rounded-xl border p-6">
      ...
      <div class="space-y-4">
        ...
        <div v-if="selectedCommand === '__custom__'">
          ...
          <input
            v-model="commandName"
            type="text"
            placeholder="Например: reboot, get_logs"
        ...
        <div>
          ...
          <textarea
            ...
            class="w-full px-4 py-3 border rounded-lg font-mono text-sm focus:ring-2 focus:ring-blue-500 outline-none"
          ></textarea>
        </div>
        ...
        >
          ...
          <pre class="text-xs font-mono text-gray-800 overflow-x-auto">{{
            previewRequest
          }}</pre>
        </div>
      ...
      <div v-if="result" class="mt-4">
        ...
        >
          ...
          <pre
            v-if="result.rawResponse && result.rawResponse !== 'null'"
            class="mt-2 text-xs bg-gray-900 text-orange-400 p-3 rounded overflow-x-auto max-h-60"
        ...
        <div v-else class="p-4 rounded-lg bg-gray-50 border border-gray-200">
          ...
          <pre
            class="text-xs bg-gray-900 text-green-400 p-3 rounded overflow-x-auto max-h-60"
            >{{ result.rawResponse || "null" }}</pre
          >
        </div>
      </div>
    </div>
...
const emit = defineEmits<{
  execute: [command: string, args: Record<string, number | string>];
}>();
...
const previewRequest = computed(() => {
  ...
  return JSON.stringify(
    { command: currentCommand.value, args: currentArgs.value },
    null,
    2
  );
});
