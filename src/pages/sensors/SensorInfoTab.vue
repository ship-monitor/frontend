        ...
        <div class="bg-white rounded-xl border p-6">
            ...
            <div class="space-y-6">
                <div>
                    ...
                    <input v-model="local.name" type="text" placeholder="Холодильник N1"
                        class="w-full px-4 py-3 border rounded-lg text-base focus:ring-2 focus:ring-blue-500 outline-none" />
                </div>

                <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1.5">Нижняя граница (C)</label>
                        <input v-model.number="local.minThreshold" type="number" step="0.5"
                            class="w-full px-4 py-3 border rounded-lg text-base focus:ring-2 focus:ring-blue-500 outline-none" />
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1.5">Верхняя граница (C)</label>
                        <input v-model.number="local.maxThreshold" type="number" step="0.5"
                            class="w-full px-4 py-3 border rounded-lg text-base focus:ring-2 focus:ring-blue-500 outline-none" />
                    </div>
                </div>
                ...
                <div>
                    ...
                    <p class="text-xs text-gray-500 mb-2">Используются для поиска и фильтрации устройств на дашборде</p>

                    <div v-if="local.tags.length > 0" class="flex flex-wrap gap-2 mb-3">
                        <span v-for="tag in local.tags" :key="tag"
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1.5">Номер телефона для SMS</label>
                    <input v-model="local.phone" type="tel" placeholder="+7XXXXXXXXXX"
                        class="w-full px-4 py-3 border rounded-lg text-base focus:ring-2 focus:ring-blue-500 outline-none" />
                </div>

                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1.5">Частота SMS-уведомлений</label>
                    <div class="relative">
                        <select v-model="local.smsFrequency"
                ...
                <div>
                    <label class="block text-sm font-medium text-gray-700 mb-1.5">Время оттайки (минут)</label>
                    <input v-model.number="local.defrostTime" type="number" min="1"
                        class="w-full px-4 py-3 border rounded-lg text-base focus:ring-2 focus:ring-blue-500 outline-none" />
                </div>

                <div class="flex justify-end gap-3 pt-4 border-t">
                    <button @click="$emit('cancel')"
                        class="px-6 py-2.5 border rounded-lg hover:bg-gray-50 text-sm touch-target">Отмена</button>
                    <button @click="$emit('save', local)" :disabled="saving"
