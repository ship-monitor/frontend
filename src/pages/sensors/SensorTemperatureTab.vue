<template>
    <div class="space-y-6">
        <div class="bg-white rounded-xl border p-6 text-center">
            ...
            <p v-if="error" class="text-xs mt-2" :class="error.includes('Ошибка') ? 'text-red-500' : 'text-gray-500'">
                {{ error }}
            </p>
        </div>

        <div class="flex flex-wrap gap-2">
            <button v-for="period in periods" :key="period.value" @click="$emit('update:period', period.value)" :class="[
...
const tempColor = computed(() => {
    ...
    if (t < props.thresholds.min || t > props.thresholds.max) return 'text-red-600'
...
watch(filteredHistory, async () => { await nextTick(); drawChart() }, { deep: true })
