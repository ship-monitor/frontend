<template>
  <div class="max-w-lg mx-auto p-4 sm:p-6">
    ...
    <div class="bg-white rounded-xl border p-5 sm:p-6 mb-6">
      <h2 class="text-base font-semibold text-gray-800 mb-4">Автообновление дашборда</h2>
      <p class="text-xs text-gray-500 mb-4">
        Статусы устройств будут автоматически проверяться через указанный интервал.
      </p>
      ...
      <div class="space-y-3">
        <label class="flex items-center justify-between p-3 bg-gray-50 rounded-lg touch-target cursor-pointer">
          <div>
            ...
            <p class="text-xs text-gray-500">Периодически проверять статус устройств</p>
          </div>
          <input v-model="settings.autoRefresh" type="checkbox"
            class="w-5 h-5 rounded border-gray-300 text-blue-500 focus:ring-blue-500 flex-shrink-0" @change="save" />
        </label>

        <div v-if="settings.autoRefresh">
          <label class="block text-xs text-gray-500 mb-1.5">Интервал обновления</label>
          <div class="relative">
            <select v-model="settings.interval" @change="save"
