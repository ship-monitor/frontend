...
<template>
  <div class="min-h-screen flex">
    ...
    <div class="hidden lg:flex lg:w-1/2 bg-ink-900 text-white relative overflow-hidden flex-col justify-between p-12">
      <div class="absolute inset-0 bg-grid opacity-30"></div>

      <!-- Логотип -->
      <div class="relative flex items-center gap-3">
        <span
          class="w-11 h-11 rounded-xl bg-brand-500 flex items-center justify-center font-bold text-lg shadow-lg shadow-brand-500/30">Ш</span>
      ...
      <div class="w-full max-w-sm animate-fade-in-up">
        ...
        <form @submit.prevent="handleLogin" class="space-y-4">
          <div>
            ...
            <ship-textbox v-model="email" placeholder="you@example.com" name="email" autocomplete="email" />
          </div>

          <div>
            <label class="block text-sm font-medium text-ink-700 mb-1.5">Пароль</label>
            <div class="relative">
              <ship-textbox v-model="password" placeholder="••••••••" name="password"
                :type="showPassword ? 'text' : 'password'" autocomplete="current-password" class="pr-11" />
        ...
        <p class="mt-6 text-center text-sm text-ink-500">
          Нет аккаунта? Запросите демо-доступ:

          <router-link :to="ROUTES.LANDING" class="text-brand-600 hover:text-brand-700 font-semibold"> Вернуться на
            лендинг
          </router-link>
          <!-- <router-link
            :to="ROUTES.REGISTER"
