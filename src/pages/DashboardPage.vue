  ...
  <div class="p-4 sm:p-6">
    ...
    >
      <div
        ...
        :class="sensor.status === 'error' ? 'border-red-200' : ''"
      ...
      >
        ...
        <div class="text-center py-3">
          ...
          <div class="text-xs text-gray-400">
            {{
              sensor.lastPingTime
                ? formatTimeAgo(sensor.lastPingTime)
                : "Нет данных"
            }}
          </div>
        </div>
        ...
        <!-- Теги -->
        <div
          v-if="sensor.tags && sensor.tags.length > 0"
        ...
        <div class="flex items-center justify-between">
          ...
          <span
            v-if="sensor.pingHistory.length > 0"
            class="flex items-center gap-0.5 shrink-0"
          >
            <span
              v-for="(ping, i) in sensor.pingHistory"
              :key="i"
              class="w-1.5 h-1.5 rounded-full"
              :class="ping ? 'bg-green-400' : 'bg-red-400'"
              :title="ping ? 'В сети' : 'Не в сети'"
            ></span>
          </span>
        </div>
      </div>
...
function statusBadgeClass(sensor: SensorDisplay) {
  ...
  if (sensor.status === "error")
    return "px-2.5 py-1 text-xs rounded-full font-medium bg-red-100 text-red-800";
...
function tempColor(sensor: SensorDisplay) {
  if (sensor.status === "offline" || sensor.status === "error")
...
function goToSensor(sensor: SensorDisplay) {
  ...
  router.push(
    route.sensorDetails(sensor.id) + "?orgId=" + sensor.organizationId
  );
}
